...
class manejador_de_ventanas:
    ...
    def pushear_a_ventana(self, index, valor):
        (se_envio, ventana) = self.ventanas[index]
        if not se_envio and ventana:
            self._pilas_respuestas.put("agrego un valor a la pila" + str(index))
            self._respuestas_escuchadas += 1
